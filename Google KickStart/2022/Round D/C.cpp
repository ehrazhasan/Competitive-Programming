...
ll dfs(int position, int i, int parent , vector<ll> &v){
	//db1(position);
	if(i == (int)v.size()) return 0;

	if(dp[{position,i}] > 0LL) return dp[{position,i}];

	ll ans = INF;
	for(int nbr : pos[v[i]]){

		ll val = dfs(nbr, i + 1, position, v);
		//db3(parent, nbr, val);
		//db3(position, nbr, val);
		ans = min(ans, abs(position - nbr) + val);
		//db2(position, ans);
	}
	///db2(position, ans);
	return dp[{position, i}] = ans;
}
