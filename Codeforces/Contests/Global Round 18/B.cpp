...
int main(){
    ...
    for(int bits = 19; bits >= 0; bits--){

    		for(int i = 1; i <= N; i++){

    			int bit = (i >> bits) & 1;
    			V[bits].push_back(bit);
   			 }
    }
    ...
    for(int bits = 19; bits >= 0; bits--){
    		...
    		for(int i = 0; i < N; i++){
    			...
    			if(V[bits][i] == 0){
    				...
    				ans[bits][i] = i - 1 >= 0 ? 1 + ans[bits][i - 1] : 1;
    			}
    			else{

    				ans[bits][i] = i - 1 >= 0 ? ans[bits][i - 1] : 0;
    			} 
    ...
    while(t--){
        ...
        for(int bit = 0; bit < 20; bit++){
        	...
        	int d = l > 1 ? ans[bit][r - 1] - ans[bit][l - 1 - 1] : ans[bit][r - 1];
