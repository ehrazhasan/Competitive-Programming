...
int main(){
    ...
    while(t--){
        ...
        multiset<pair<ll,ll>,greater<pair<ll,ll>>> v;
        rep(i,n){
        	ll a;
        	cin >> a;
        	if(a == 0) continue;
        ...
        while(v.size() >= 2){
        	...
        	ll pos2 = it2->second;
        	v.erase(it2);
        	//db2(it1->first,it2->first);
        	ll x1 = el1 - 1;
        	ll x2 = el2 - 1;

      		//p1.push_back(min(el1,el2));
        	//ans += min(el1,el2);
        	//db2(it1->second,it2->second);
        	p.push_back({pos1,pos2});
