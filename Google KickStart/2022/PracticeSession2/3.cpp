...
int main(){
    ...
    for(int tt = 1; tt <= t; tt++){
    	...
    	for(int i = 0; i < s.length(); i++){
    		...
    		if(vowel.find(s[i]) != -1) vowelPos.push_back(i); 
    	}

    	int sz = vowelPos.size();
    	...
    	for(int i = 1; i + 3 < sz; i++){
    		...
    		string start = s.substr(vowelPos[i - 1], vowelPos[i] - vowelPos[i - 1] + 1);

    		for(int j = i + 3; j < sz; j++){

    			string end = s.substr(vowelPos[j - 1], vowelPos[j] - vowelPos[j - 1] + 1);
    			//db2(start, end);
    			if(start == end){
