...
int findComp(int v, vector<int> &par){

	if(v != par[v]) par[v] = findComp(par[v],par);
...
int main(){
    ...
    while(t){
        ...
        for(auto it : edge){
        	...
        	int findParentu = findComp(u,par);
        	int findParentv = findComp(v,par);

        	if(findParentv == findParentu) continue;

        	maxEdgeInMST--;
        	par[findParentu] = par[findParentv];
        	ans += wt;
        }
