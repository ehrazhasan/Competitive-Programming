...
ll dfs(ll pos, ll weight){
	...
	if(dp[pos][weight] != INF) return dp[pos][weight];

	ll inc = 0LL, exc = 0LL;
	if(weight - moves[pos] >= 0)
		inc = cost[pos] + dfs(pos + 1,weight - moves[pos]);
	exc = dfs(pos + 1, weight);

	ll ans = max(inc,exc);
	//db1(ans);

	return dp[pos][weight] = ans;
...
int main(){
    ...
    while(t--){
    	...
    	//The problem reduces to find the maximum profit I can attain such that
    	//I use at max k operations

    	// dp[i][k] -> the profit so far If I am standing at ith index and used k operation
