    ...
    cout.tie(0);
   	...
   	while(q--){
   		...
   		if(p.empty()){
   			...
   			if(a + 2 < n and s.substr(a, 3) == "abc"){

				total++;
				p.insert(a + 2);
   			}

			if(a - 2 >= 0 and s.substr(a - 2, 3) == "abc"){

				total++;
				p.insert(a);
			}

			if(a - 1 >= 0 and a + 1 < n and s[a - 1] == 'a' and s[a] == 'b' and s[a + 1] == 'c'){
				total++;
				p.insert(a + 1);
			}
   		...
   		auto it = p.lower_bound(a);
			...
			int el = *it;
			//db1(el);
			if(a == el or a == el - 1 or a == el - 2){

				if(s.substr(el - 2, 3) == "abc"){
