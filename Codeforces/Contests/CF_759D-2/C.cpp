...
int main(){
    ...
    while(t--){
        ...
        vector<ll> v(n),positive,negative,st;
        rep(i,n) {

        	cin >> v[i];

        	if(v[i] >= 0) positive.push_back(v[i]);
        	else negative.push_back(v[i]);
        	
        }

        sort(all(positive));

        int p = (int)positive.size();
        int q = (int)negative.size();

       	if(p > 0){

       		int pos = p - 1;

       		st.push_back(positive[pos]);
       		pos -= k;
       	...
       	sort(all(negative),greater<ll>());

       	if(q > 0){

       		int pos = q - 1;

       		st.push_back(abs(negative[pos]));
       		pos -= k;
