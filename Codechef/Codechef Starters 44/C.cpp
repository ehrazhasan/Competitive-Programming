...
int main(){
    ...
    while(t--){
        ...
        for(int i = 1; i < n; i++) pref[i] = pref[i - 1] + mexes[i];

        for(ll i = 0; i < n; i++){

        	auto it = upper_bound(all(mexes), i);
        	int gThanMex = mexes.end() - it;
        	int gLessMex = n - gThanMex;
        	int index = it - mexes.begin();


        	auto it1 = lower_bound(all(mexes), i);
        	int ind = it1 - mexes.begin();

        	ll least = gThanMex;
        	ll a = gThanMex*n + gThanMex - (pref[n - 1] - (index > 0 ? pref[index - 1] : 0));
        	
     	   	ll more = a + ind*n - (ind > 0 ? pref[ind - 1] : 0);
