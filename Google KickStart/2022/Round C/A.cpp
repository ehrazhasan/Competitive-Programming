...
int main(){
    ...
    for(int tt = 1; tt <= t; tt++){
        ...
        bool haveSP = false, l = false, u = false, d = false;

        for(int i = 0; i < n; i++){

        	if(s[i] == '@' or s[i] == '&' or s[i] == '*' or s[i] == '#') haveSP = true;
        	if(s[i] >= '0' and s[i] <= '9') d = true;
        	if(s[i] >= 'A' and s[i] <= 'Z') u = true;
        	if(s[i] >= 'a' and s[i] <= 'z') l = true;
        }
