...
ll calc(int n, vector<vector<ll>> &v, int i, int j){
	...
	while(i + k < n){
		...
		if(mp.size() != 2){
			...
			ll freq1 = 0; ll freq3 = 0;

			for(auto it : mp){

				if(it.second == 1) freq1 = it.first;
				if(it.second == 3) freq3 = it.first; 
			}

			if(freq3 == 0 or freq1 == 0){

				ans1 = -1;
			}
			else{

				if(freq3 < freq1) ans1 = freq3;
				else ans1 = -1;
			}
		}
