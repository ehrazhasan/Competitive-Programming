...
ll solve(int i, int j){
	...
	return dp[i][j] = max(aa[i][j] + solve(i - 1, j), bb[i][j] + solve(i, j - 1));
}
...
int main(){
    ...
    while(t--){
        ...
        b = vector<vector<ll>>(n, vector<ll>(m));
        aa = vector<vector<ll>>(n, vector<ll>(m));
        bb = vector<vector<ll>>(n, vector<ll>(m));
