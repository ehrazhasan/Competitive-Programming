...
int main(){
    ...
    while(t--){
        ...
        cin >> n >> k;
       	...
       	function<bool(int,int,int)> isPossible = [&](int x, int n, int par) {

       		if(x == 1){

       			if(n > 0 and n % 2 == par) return true;
       			return false;
       		}

       		if(n <= 0) return false;

       		if(isPossible(x - 1, n - (par == 0 ? 2 : 1), par)) return true;
       		return false;
       	};
