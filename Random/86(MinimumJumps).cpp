...
public:
    int minimumJumps(vector<int>& forbidden, int a, int b, int x) {
        map<int,bool> forward;
        map<int,bool> backward;
        set<int> f(forbidden.begin(),forbidden.end());
    
        int maxi = a+b+max(x,*f.rbegin());
        //db1(maxi);
        ...
        while(!q.empty()){
          ...
          for(int i = 0; i < n; i++){
             ...
             // db3(ce,w,r);
              if(w >= 0 and f.count(w) == 0 and w <= maxi and !forward[w]){
                forward[w] = true;
                q.push({w,false});
              }
              if(r >= 0 and f.count(r) == 0 and r <= maxi and !backward[r] and !flag){
                backward[r] = true;
                q.push({r,true});
              }             
          }
