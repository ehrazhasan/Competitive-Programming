    ...
    while(t--){
      	...
      	rep(i,n){
      		...
      		rep(j,m){
      			...
      			if(a[v[i][j]].size() == 0)
      				a[v[i][j]].push_back({i + 1, j + 1});
      			else{

      				ans += (mp[v[i][j]]*2-2) * (abs(i + 1 - a[v[i][j]].back().first) + abs(j + 1 - a[v[i][j]].back().second));
      				a[v[i][j]].push_back({i + 1, j + 1});
      			}
      		}
      	}
      		...
      		if(a[i].size() > 0){
      			...
      			for(int k = 0; k < a[i].size() ; k++){
      				...
      				for(int j = 0; j < a[i].size(); j++){
      					...
      					if(k == j) continue;
      					ans1 += abs(a[i][k].first - a[i][j].first) + abs(a[i][k].second - a[i][j].second);  
