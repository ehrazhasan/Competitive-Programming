...
int main(){
    ...
    while(t--){
        ...
        map<ll, int> present;
        for(int i = 0; i < n; i++) present[b[i]] = i + 1;
        ...
        for(int i = m - 1; i >= 0; i--){
        	...
        	if(i == m - 1){
        		...
        		//if it is not already painted, paint it;
        		if(positions[c[i]].size() > 0){

        			int pos = positions[c[i]].back();
        		...
        		//if it is already painted or not there then just paint the next plank
        ...
        for(int i = 0; i < m; i++) a[ans[i] - 1] = c[i];
