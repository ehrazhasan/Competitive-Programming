...
int main(){
    ...
    for(int tt = 1; tt <= t; tt++){
        ...
        for(int i = 0; i < n; i++){
        	...
        	auto it = upper_bound(all(p), 2LL * curr);

        	if(it == p.begin()) ans[i] = -1;
        	else if(it == p.end()){

        		it--;
        		if(p[it - p.begin()] == curr){

        			if(m[curr] > 1) ans[i] = curr;
        			else if(it == p.begin()) ans[i] = -1;
        			else ans[i] = p[prev(it) - p.begin()];
	        	}
	        	else{
