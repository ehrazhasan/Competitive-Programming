...
int main(){
    ...
    while(t--){
        ...
        for(int i = 0; i < n; i++){
        	...
        	int x = upper_bound(all(ind),i) - ind.begin();

        	int y = lower_bound(all(ind),i+n) - ind.begin();

        	//db2(x,y);

        	int time1 = abs(i - ind[x]);
        	int time2 = abs(i+n - ind[y-1]);

        	//db3(v[i],time1,time2);

        	int minTime = min(time2,time1);
