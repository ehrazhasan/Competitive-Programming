...
int main(){
    ...
    while(t--){
        ...
        map<ll, ll> m;
        vector<ll> v(n);
        ll zeros = 0, positive = 0, negative = 0;
        rep(i,n){
        ...
        if(n == 3){
        	...
        	ll sum = v[0] + v[1] + v[2];
        	if(sum == v[0] or sum == v[1] or sum == v[2]){
        ...
        if((positive == 2 and zeros > 0) or (negative == 2 and zeros > 0)){
        ...
        for(int i = 0; i < n; i++){
        	...
        	if(v[i] == 0 and cnt < 4) p.push_back(0);
