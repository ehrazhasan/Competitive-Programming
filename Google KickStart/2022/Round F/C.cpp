...
int main(){
    ...
    while(t--){
    	...
    	ll d, n, x;
    	cin >> d >> n >> x;
    	vector<pair<ll, pair<ll,ll>>> seeds(n); //q, l, v
    	rep(i, n){
    	...
    	sort(all(seeds), [&](const pair<ll, pair<ll,ll>> &a, pair<ll, pair<ll,ll>> &b){
    	...
    	for(int bitmask = 1; bitmask < (1 << n); bitmask++){
			...
			for(int i = 0; i < n; i++){
				...
				if(((bitmask >> i) & 1) and currDay + seeds[i].second.first <= d){
					//db3(currDay, seeds[i].second.first, seeds[i].second.second);
					cnt += seeds[i].second.second;
					currDay += 1;
				}
			}
