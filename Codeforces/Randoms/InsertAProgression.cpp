    ...
    while(t--){
        ...
        for(int i = 0; i < n; i++){
        	...
        	if(i == 0){
        		...
        		if(extra > abs(v[n - 1] - 1)){

        			extra = abs(v[n - 1] - 1);
        			pos = n - 1;
        		}

        		if(extra > abs(abs(v[i] - v[i - 1]) - (abs(v[i] - 1) + abs(v[i - 1] - 1)))){

        			extra = abs(abs(v[i] - v[i - 1]) - (abs(v[i] - 1) + abs(v[i - 1] - 1)));
        			pos = i;
        ...
        if(x > mx){
        	...
        	for(int i = 0; i < n; i++){
	        	...
	        	if(i == 0){
	        		...
	        		if(extra > abs(v[n - 1] - x)){

	        			extra = abs(v[n - 1] - x);
	        			pos = n - 1;
	        		}

	        		if(extra > abs(abs(v[i] - v[i - 1]) - (abs(v[i] - x) + abs(v[i - 1] - x)))){

	        			extra = abs(abs(v[i] - v[i - 1]) - (abs(v[i] - x) + abs(v[i - 1] - x)));
	        			pos = i;
