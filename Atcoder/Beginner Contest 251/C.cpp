...
int main(){
    ...
    while(t--){
      	...
      	vector<ll> submissions(n + 1, INT_MAX);
      	map<string, int> m;

      	vector<ll> bestSubmissions(n + 1);
      	...
      	repA(i,1,n){
      		...
      		if(!m[s]){
      			...
      			m[s] = i;
      			bestSubmissions[i] = time;
      		}
      		
      	} 

      	ll ans = 1;
      	ll soFar = bestSubmissions[1];

      	repA(i,1,n){
      		//db1(bestSubmissions[i]);
      		if(bestSubmissions[i] > soFar) {
      			ans = i;
      			soFar = bestSubmissions[i];
      		}
      	}
