...
/*
	...
	I have to choose a d such that 0 <= 3 * d <= hi

	Decrease hi -> 3*d;
	Increase h(i-1) -> d;
	Increase h(i - 1) -> 2*d;

	For different value of i I can change the value of d or can keep it same

	I have to maximize the minimum number of stones in the heap 

	These kind of questions can be solved using BS
...
bool isPossible(ll N){
	...
	for(int i = n - 1; i > 1; i--){
		...
		ll d = min(h[i],(a[i] - N)) / 3;
		
		a[i - 1] += d;
		a[i - 2] += 2 * d;
	}

	ll el = *min_element(all(a));
