...
int main(){
    ...
    while(t--){
        ...
        vector<int> cntOnes(n, 0), cntZeros(n, 0);

        for(int i = 0; i < n; i++){

            if(i == 0){

                cntZeros[i] = s[i] == '1' ? 0 : 1;
                cntOnes[i] = s[i] == '1' ? 1 : 0;
            }
            else{
                ...
                if(s[i] == '1'){
                    ...
                    cntOnes[i] = 1 + cntOnes[i - 1];
                    cntZeros[i] = cntZeros[i - 1];
                }
                else{

                    cntZeros[i] = 1 + cntZeros[i - 1];
                    cntOnes[i] = cntOnes[i - 1];
                }
            }
        }
        ...
        for(int i = 0; i < n; i++){
            ...
            if(ch == 0){
               ...
               ans = min(ans, cntOnes[i] + (cntZeros[n - 1] - cntZeros[i]));
            }
            else{

               ans = min(ans, cntZeros[i] + (cntOnes[n - 1] - cntOnes[i]));
            }
        }
