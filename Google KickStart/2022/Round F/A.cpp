...
int main(){
    ...
    while(t--){
        ...
        vector<pair<string, pair<int,int>>> fabrics(n);
        ...
        for(int i = 0; i < n; i++) c[i] = fabrics[i];

        sort(all(c), [&](const pair<string, pair<int, int>> &a, pair<string, pair<int, int>> &b){

        	if(a.first != b.first) return a.first < b.first;
        	return a.second.second < b.second.second;
        });

    	vector<pair<string, pair<int, int>>> d(n);
        for(int i = 0; i < n; i++) d[i] = fabrics[i];

    	sort(all(d), [&](const pair<string, pair<int, int>> &a, const pair<string, pair<int, int>> &b){

    		if(a.second.first == b.second.first) return a.second.second < b.second.second;
    		return a.second.first < b.second.first;
    	});

    	//for(int i = 0; i < n; i++) cout << c[i].first << " " << c[i].second << " " << d[i].first << " " << d[i].second << endl;
