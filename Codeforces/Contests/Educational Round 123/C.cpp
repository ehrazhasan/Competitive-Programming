...
/*
	...
	f(k) : maximum sum of a contiguous subarray
		   add x to the elements on exactly k distinct positions.


		   K = 0 -> max subarray sum of original array

	Note: since we want to maximize the sum, the new sum will either
...
int main(){
    ...
    while(t--){
     	...
     	for(ll i=1;i<=n;i++) 
     	{
     		...
     		for(ll j=i;j<n;j++)
     		{
     			sum = max(sum,preSum[j]-preSum[j-i]);
     		}
     		lengthSum[i]=sum;
     	}
     	...
     	for(ll i = 0;i<=n;i++)
     	{
     		...
     		for(ll j=1;j<=n;j++)
     		{
     			ans = max(ans,lengthSum[j]+min(i,j)*x);
     		}
