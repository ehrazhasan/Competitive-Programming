...
ll solve(string &number, ll n, ll x, bool leadingZero, bool tight){

	if(n == 0) return 1LL;


	if(dp[n][x][leadingZero][tight] > -1) return dp[n][x][leadingZero][tight];
	...
	for(int d = lb; d <= ub; d++){
		...
		if(d == x and !leadingZero) continue;
		ans += solve(number, n - 1, d, leadingZero & (d == 0), tight & (d == ub));
	}
...
int main(){
    ...
    while(t--){
      	...
      	ll ans1 = solve(B, (int)B.length(), 0, 1, 1);
      	memset(dp, -1, sizeof dp);
      	ll ans2 = solve(A, (int)A.length(), 0, 1, 1);
