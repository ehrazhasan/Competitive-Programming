...
/*
	...
	ith friend has i dollars

	ith friend will be happy if:
	1) atmost ai people are strictly richer than him 
	2) atmost bi people are strictly poorer than him


	let's say for ith friend

	total n - i - 1 people are richer than him
	total i people are poorer than him
...
bool isPossible(ll mid,ll n){
	...
	for(int i = 0; i < n; i++){
		...
		if(bi[i] >= cnt and mid - cnt - 1 <= ai[i]) cnt++;
	}
