...
int calc(string s){
    	...
    	for(int i = 0; i < n ; i++){
    		...
    		if((odds > evens + d) or (evens > odds + c)){
...
int main(){
    ...
    while(t--){
        ...
        if(totalPossibilities == 1){
        	...
        	if(!gotit) cout << n << endl;
        	continue;
        }
        //db1(totalPossibilities);
        int ans = 10;

        for(int p = 1; p <= totalPossibilities; p++){
