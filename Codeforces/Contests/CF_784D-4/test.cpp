...
int main(){
    ...
    while(t--){
        ...
        potential_centers.push_back(p[0].second);

        for(int i = 1; i < p.size(); i++){

        	if(p[i].first == mini) potential_centers.push_back(p[i].second);
        }


        for(auto it : potential_centers) cout << it << " ";
        cout << endl;

    	int ans = potential_centers[0];
    	int in = indegree[potential_centers[0]];


    	for(auto it : potential_centers){
    	...
    	cout << "Center node with highest degree : " << ans << endl;
