...
/*
	...
	dp[i][j] = the count of sequences that satisfies the condition given that the sequence length is 
			   i and the ith value is j

	dp[1][j] = 1

	dp[i][j] -> dp[i - 1][1 ............ j - k] -> dp[j + k .................... M]
...
int main(){
    ...
    while(t--){
      	...
      	for(int i = 1; i <= m; i++){
      		...
      		dp[1][i] = 1;
      		pref[1][i] = mod_add(pref[1][i - 1], dp[1][i], MOD1);
      	}
      	...
      	for(int i = 2; i <= n; i++){
      		...
      		for(int j = 1; j <= m; j++){
      			...
      			if(j - k >= 1){
      				...
      				dp[i][j] = mod_add(dp[i][j], pref[i - 1][j - k], MOD1);
      			}

      			if(j + k <= m){

      				dp[i][j] = mod_add(dp[i][j], mod_sub(pref[i - 1][m], pref[i - 1][j + k - 1], MOD1), MOD1);
      			}
      		}

      		for(int k = 1; k <= m; k++){

      			pref[i][k] = mod_add(pref[i][k - 1], dp[i][k], MOD1); 
      		}
      	}

      	ll ans = 0;

      	for(int i = 0; i <= m; i++) ans = mod_add(ans, dp[n][i], MOD1);
