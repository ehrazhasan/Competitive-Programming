...
/*
	...
	We have to do k partitions
	badness = abs(zeros - ones);
		
	cnt0 -> zeros
	cnt1 -> ones

	I want to make this difference as small as possible
...
int main(){
    ...
    while(t--){
        ...
        if(rem1 > rem2){
        	...
        	cout << max(abs(val1 - val2), abs(val1 + 1 - val2)) << endl;
        }
        else{

        	cout << max(abs(val1 - val2), abs(val2 + 1 - val1)) << endl;
        }
    }
    return 0;
}