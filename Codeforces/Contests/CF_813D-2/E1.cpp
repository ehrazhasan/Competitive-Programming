...
int main(){
    ...
    while(t--){
    	...
    	for(int i = 3; i <= r; i++){
    		...
    		ll totalComb = combination(i - 1, 2LL, MOD, fact, ifact);

    		ll sz = divs[i].size();

    		ll comb = (sz <= 2 ? 0LL : combination(sz - 1, 2LL, MOD, fact, ifact));
    		...
    		for(int j = 0; j < sz; j++){
    			...
    			for(int k = j + 1; k < sz; k++){
    				...
    				//if(i == 10) db2(divs[i][j], divs[i][k]);
    				if(LCM(divs[i][j], divs[i][k]) < 1 + divs[i][j] + divs[i][k]) cnt++;
    			}
    		}
    		...
    		// db3(totalComb, sz, comb);



    		dp[i] = totalComb  + dp[i - 1] - cnt;
    		//db1(dp[i]);
    	...
    	//db1(combination(7LL, 2LL, MOD, fact, ifact));
    	// db1(divs[5].size());
        ...
        for(ll i = 1; i <= n; i++){
        	...
        	for(ll j = 1; j <= n; j++){
        		...
        		for(ll k = 1; k <= n; k++){
        			...
        			if(i < j and j < k and i < k and lcm(i, j, k) >= (i + j + k)){
