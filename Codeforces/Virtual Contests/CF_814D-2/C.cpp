...
int main(){
    ...
    while(t--){
        ...
        int pos = max_element(all(v)) - v.begin();
        ...
        int done = accumulate(all(wins), 0);
        // for(int i = 0; i <= n; i++) cout << wins[i] << " ";
        // cout << endl;

        vector<int> pref(n + 1, 0);
        for(int i = 1; i <= n; i++) pref[i] = pref[i - 1] + wins[i];
        ...
        rep(j, q){
            ...
            int roundsDone = pref[v[i]];
            int roundsWin = wins[v[i]];
            int roundsNotWon = roundsDone - roundsWin;

            if(round >= roundsDone){

                cout << roundsWin + (v[i] == n ? round - roundsDone : 0) << endl;
            }
            else{

                if(roundsNotWon >= round) cout << 0 << endl;
                else cout << round - roundsNotWon << endl;
            }
        }
    }
    return 0;
