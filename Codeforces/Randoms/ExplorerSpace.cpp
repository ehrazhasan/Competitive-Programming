...
int main(){
    ...
    while(t--){
        ...
        vector<vector<ll>> v1(n,vector<ll>(m - 1));
        vector<vector<ll>> v2(n - 1,vector<ll>(m));
		...
		vector<vector<vector<ll>>> dp(n + 5,vector<vector<ll>>(m + 5,vector<ll>(k + 5)));
			...
			for(int i = 0; i < n; i++){
				...
				for(int j = 0; j < m; j++){
					...
					if(j + 1 < m){
						...
						dp[i][j][K] = min(dp[i][j][K],2*v1[i][j] + dp[i][j + 1][K-2]);
					}

					if(j - 1 >= 0){

						dp[i][j][K] = min(dp[i][j][K],2*v1[i][j - 1] + dp[i][j - 1][K-2]);
					}

					if(i - 1 >= 0){

						dp[i][j][K] = min(dp[i][j][K],2*v2[i - 1][j] + dp[i - 1][j][K-2]);
					}
					if(i + 1 < n){

						dp[i][j][K] = min(dp[i][j][K],2*v2[i][j] + dp[i + 1][j][K-2]);
					}
				}
			}
		}
