...
bool isPossible(ll mid, ll n, vector<ll> &pref, ll maxi,vector<pair<ll,ll>> &p){

	ll sum = pref[mid];

	if(sum >= maxi) return true;

	for(ll i = mid + 1; i < n; i++){

		if(sum < p[i].first) return false;
		sum += p[i].first;
	}
...
int main(){
    ...
    while(t--){
        ...
        while(s <= e){
        	...
        	if(isPossible(mid,n,pref,maxi,p)){
        ...
        for(ll i = pos; i < n; i++) ans.push_back(p[i].second);
