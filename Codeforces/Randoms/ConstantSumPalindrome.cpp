...
int main(){
    ...
    while(t--){
        ...
        vector<ll> cnt(2 * k + 1);
        vector<ll> mins(n / 2), maxs(n / 2);

        for(int i = 0; i < n / 2; i++){

            maxs[i] = max(a[i], a[n - i - 1]) + k;
            mins[i] = min(a[i], a[n - i - 1]) + 1;
            cnt[a[i] + a[n - i - 1]]++;
        }
        ...
        for(ll x = 2; x <= 2 * k; x++){
            ...
            ll a = mins.end() - upper_bound(all(mins), x);
            ll b = lower_bound(all(maxs), x) - maxs.begin();

            ll c2 = a + b;
            ll c0 = cnt[x];
            ll c1 = n / 2 - c2 - c0;
            ll currAns = c0 * 0 + c1 * 1 + c2 * 2;
            ans = min(ans, currAns);
