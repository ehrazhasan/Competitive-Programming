...
int main(){
    ...
    while(t--){
        ...
        suff[n - 1] = s[n - 1] == '0' ? 0 : 1;
        for(int i = n - 2; i >= 0; i--){

        	suff[i] = s[i] == '0' ? suff[i + 1] : 1 + suff[i + 1];
        }
        ...
        for(int i = 1; i < n; i++){
        	...
        	pref1[i] = s[i] == '0' ? pref1[i - 1] : 1 + pref1[i - 1];
        }

        vector<ll> suff1(n);

        suff1[n - 1] = s[n - 1] == '0' ? 0 : 1;
        for(int i = n - 2; i >= 0; i--){

        	suff1[i] = s[i] == '0' ? suff1[i + 1] : 1 + suff1[i + 1];
        }

        for(int i = 0; i < n; i++){

        	ll a = pref1[i];
        	ll b = i + 1 < n ? suff1[i + 1] : 0LL;
        	if(a == b) d[1]++;
        }
     ...
     //    ll zeros = count(all(pref), 0);

     //    if(zeros) d = max(1LL, d - 1);
