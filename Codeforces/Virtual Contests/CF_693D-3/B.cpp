...
bool equalSum(vector<ll>& v, int ind, ll curSum, ll& target, vector<ll>& dp) {
	if(curSum == target) {
		return true;
	}

	if(curSum > target || ind == v.size()) {
		return false;
	}
	...
	bool pick = false;
	pick = equalSum(v, ind+1, curSum+v[ind], target, dp);

	bool notpick = equalSum(v, ind+1, curSum, target, dp);
