...
// ll ways(int pos, int n, vector<int> &f, ll s, ll sum, ll k){
...
// 	if(taken < k){
// 		taken++;
// 		inc = ways(pos + 1, n, f, s + f[pos], sum,k);
// 		taken--;
// 	}
// 	ll exc = ways(pos + 1, n, f, s, sum,k);

// 	//db2(inc,exc);

// 	ll ans = mod_add(inc,exc,MOD);
    ...
    while(t--){
        ...
        for(int i = 0; i < n; i++) {
    		...
    		if(m[f[i]] >= k and k != 0){	
    			...
    			ll val = combination(m[f[i]],k,MOD,fact,ifact);
        ...
        // Now we have to find out the number of ways such that 
        // it sums up to sum
