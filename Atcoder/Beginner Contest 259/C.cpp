...
int main(){
    ...
    while(t--){
      	...
      	if(s == p) cout << "Yes";
      		...
      		map<char, queue<int>> s1, p1;
      		for(int i = 0; i < s.length(); i++){

      			char curr = s[i];
      			int cnt = 0;
      			int j = i;
      			while(i < s.length() and curr == s[i]){
      		...
      		for(int i = 0; i < s.length(); i++){
      			...
      			//db2(s1[s[i]].front(), p1[p[i]].front());
      			if(s1[s[i]].front() != p1[p[i]].front() and (s1[s[i]].front() == 1 or s1[s[i]].front() > p1[p[i]].front())){
