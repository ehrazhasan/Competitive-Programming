...
int main(){
    ...
    while(t--){
        ...
        rep(i, n - 1){
        	...
        	if(u == x) special[v] = true;
        	if(v == x) special[u] = true;
        ...
        while(true){
        	...
        	if(leafNodes.find(x) != leafNodes.end()) break;

        	bool okay = false;
        	for(int leafNode : leafNodes){

        		if(!special[leafNode] or indegree[x] - 1 > 1){

        			for(int nbr : edges[leafNode]){

        				indegree[nbr] -= 1;
        				if(indegree[nbr] == 1) leafNodes.insert(nbr);
        			}

        			//db1(leafNode);

        			f ^= 1;
        			leafNodes.erase(leafNode);
        			okay = true;
        			break;
        		}
