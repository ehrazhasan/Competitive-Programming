...
int main(){
    ...
    while(t--){
        ...
        vector<vector<ll>> pref(2, vector<ll>(n + 2, 0LL));

        for(int i = 1; i <= n; i++){

        	pref[0][i] = s[i - 1] == '1' ? pref[0][i - 1] : pref[0][i - 1] + 1;
        	pref[1][i] = s[i - 1] == '1' ? pref[1][i - 1] + 1 : pref[1][i - 1]; 
        }

        auto get = [&](int i, int j, int x){
        	return pref[x][j] - pref[x][i];
        };
        ...
        ll ans = n;
      	...
      	for(int i = 0; i <= n + 1; i++){
      		...
      		while(start <= end){
      			...
      			// db1(i);
      			// db3(start, mid, end);
      			if(get(mid, i - 1, 0) > pref[1][n] - get(mid, i - 1, 1)){
      		...
      		ans = min(ans, max(get(start, i - 1, 0), pref[1][n] - get(start, i - 1, 1)));
      	}  
