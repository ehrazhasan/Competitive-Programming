...
int n;
ll dp[315][315][315];
ll solve(ll i,vector<vector<int>> &v , ll a, ll b){

    if(a >= x and b >= y) return 0;
    if(i >= n) return 1e12;

    if(dp[i][min(a,305LL)][min(b,305LL)] != -1) return dp[i][min(a,305LL)][min(b,305LL)];

  
    
    ll ans = min(solve(i+1,v,a,b),1 + solve(i+1,v,a + (ll)v[i][0],b + (ll)v[i][1]));
    

    return dp[i][min(a,305LL)][min(b,305LL)] = ans;
