...
/*
	...
	Total subarray possible = (n * (n + 1)) / 2	
	
	Goal : I have to find the array of length N such that it has 
		   exactly K subarray
...
ll n, k;
vector<ll> c;
bool solve(ll len, ll cnt){

	if(len == 0 and cnt == 0) return true;

	if(len == 0 or len > cnt) return false;
