...
int main(){
    ...
    while(t--){
        ...
        vector<int> v(n);
        rep(i,n) { cin >> v[i]; m[v[i]]++;}

        bool okay = false;
        for(int i = 0; i + 3 - 1 < n; i++){

        	if(isPalindrome(v,i,i + 3 - 1)){
        	...
        	for(auto it : m){
        		...
        		if(it.second == 2){
        			...
        			for(int i = 0; i < n; i++){
        				...
        				if(v[i] == it.first and pos == -1){

        					pos = i;
        				}
        				else if(v[i] == it.first){
        			...
        			if(pos1 != -1 and pos != -1 and abs(pos1 - pos) > 1){
