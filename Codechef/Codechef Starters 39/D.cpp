...
vector<ll> dfsSize(N), tmp;

ll dfs_size(ll node, ll parent){
...
void dfs(ll node, ll parent = 0){
	...
	for(auto nbr : edges[node]){
		...
		if(nbr.second == 1){
			...
			tmp.push_back(dfsSize[nbr.first]);
			continue;
		}
...
int main(){
    ...
    while(t--){
    	...
    	while(totalInfected > k and tmp.size() > 0){

    		totalInfected -= tmp.back();
    		tmp.pop_back();
    		ans++;
    	}

    	if(totalInfected > k) ans = -1;
    	cout << ans << endl;


    	// for(int i = 1; i <= n; i++) cout << dfsSize[i] << " ";
    	// cout << endl;
