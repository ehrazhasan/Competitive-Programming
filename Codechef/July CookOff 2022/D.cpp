...
int main(){
    ...
    while(t--){
        ...
        for(auto it : freq){
        	...
        	ans = mod_add(ans, expo(2LL, f, MOD), MOD);
        	ans = mod_sub(ans, f + 1LL, MOD);
        	...
        	for(int j = 1; j * j <= val; j++){
        		...
        		if(val % j == 0){
        			...
        			cnt[j] += f;
        			if(j > 1 and f >= j){
        				ans = mod_sub(ans, combination(f, j, MOD, fact, ifact), MOD);
        			}

        			if((val / j) != j and (val / j) <= n){

        				int a = val / j;
        				cnt[a] += f;	
        				if(a > 1 and f >= a) ans = mod_sub(ans, combination(f, a, MOD, fact, ifact), MOD);
        			}
        		}
        	}
        }
        ...
        for(int len = 2; len <= n; len++){
        	...
        	if(cnt[len] < len) continue;
        	ans = mod_add(ans, combination(cnt[len], len, MOD, fact, ifact), MOD);
        }
