...
const int N = 1e5 + 5;
ll start, en;
vector<pair<ll,pair<ll,ll>>> edge[N];
...
void dijikstra(int n){
	...
	while(st.size() > 0){
		...
		for(auto p : edge[vertex]){

			ll v = p.first;
			ll wt = p.second.first;
			ll time = p.second.second;

			if(dist[vertex] <= time){

				ll curr_dist = dist[vertex] + wt;
				if(curr_dist < dist[v]){
...
int main(){
    ...
    while(t--){
        ...
        for(int i = 0; i < m; i++){
        	...
        	vector<ll> weights(k - 1);
        	rep(l,k-1) cin >> weights[l];

        	ll time = last_second;
        	for(int v = 0; v < k - 1; v++){

        		edge[nodes[v]].push_back({nodes[v + 1],{weights[v],time}});
        		time += weights[v];
        	} 
        }
