    ...
    while(t--){
    	...
    	rep(i, m){
    		...
    		if(sqDist - rh <= rs) {
    			...
    			blues.push_back(dist);
    		}
    	}   

    	if(sz(reds) == 0 and sz(blues) == 0){
    	...
    	if(sz(reds) == 0){
    		...
    		cout << 0 << " " << sz(blues) << endl;
    		continue;
    	}

    	sort(all(reds));
    	sort(all(blues));
    	// for(auto it : reds) cout << it << " ";
    	// cout << endl;
    	// for(auto it : blues) cout << it << " ";
    	// cout << endl;

    	b = (lower_bound(all(blues), reds[0])) - blues.begin();
    	r = (lower_bound(all(reds), blues[0])) - reds.begin();
