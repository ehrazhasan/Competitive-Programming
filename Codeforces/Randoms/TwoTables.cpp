...
int main(){
    ...
    while(t--){
        ...
        ld ans1 = INT_MAX, ans2 = INT_MAX, ans3 = INT_MAX, ans4 = INT_MAX;
        ...
        // left side
        if(width + x2 - x1 <= W) ans1 = abs(width - x1);

        // upper side

        if(height + y2 - y1 <= H) ans2 = abs(y2 - (H - height));

        // right side

        if(width + x2 - x1 <= W) ans3 = abs(x2 - (W - width));

        // down side

        if(height + y2 - y1 <= H) ans4 = abs(height - y1);

        ld toPRINT = min({ans1,ans2,ans3,ans4});
