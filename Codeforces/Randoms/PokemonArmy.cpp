...
ll solve(vector<ll> &v, int n, int start, int f){

    if(start == n) return 0;

    if(dp[start][f] != -1) return dp[start][f];

    ll ans1 = 0;
    ans1 = solve(v,n,start + 1,f);

    ll ans2 = 0;

    if(f == 0){

        ans2 += v[start] + solve(v,n,start + 1, 1);
    }
    else{

        ans2 += -1LL * v[start] + solve(v,n,start + 1, 0);
    }

    ll ans = max(ans2,ans1);

    return dp[start][f] = ans;
} 
