...
int main(){
    ...
    while(t--){
        ...
        for(int i = 1; i <= n; i++){
        	...
        	diff[i - el].push_back(i);
        	sum[i + el].push_back(i);
        }
        ...
        for(int i = 1; i <= n; i++){
        	...
        	auto it = upper_bound(diff[d].begin(),diff[d].end(),i);
        	if(it != diff[d].end()){
        		int index = it - diff[d].begin();
        		ans += diff[d].size() - index;
        	}
        	//else{

        		auto it1 = upper_bound(sum[su].begin(),sum[su].end(),i);
        		if(it1 != sum[su].end()){
					int index = it1 - sum[su].begin();
	        		ans += sum[su].size() - index;
        		}
        	//}
