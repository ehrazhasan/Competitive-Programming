...
/*
	...
	prefix[a] + suffix[a] + prefix[b] + suffix[b];
...
int main(){
    ...
    for(int tt = 1; tt <= t; tt++){
        ...
        for(int i = 0; i <= toRemove; i++){
        	...
        	if(wsb > m or wsa > n) continue;

        	ll fromA = INF, fromB = INF;

        	for(int j = 1; j + wsa - 1 <= n; j++){

        		ll sum = prefA[j + wsa - 1] - prefA[j - 1];
        		fromA = min(fromA, sum);
        	}

        	for(int j = 1; j + wsb - 1 <= m; j++){

        		ll sum = prefB[j + wsb - 1] - prefB[j - 1];
        		fromB = min(fromB, sum);
        	}
