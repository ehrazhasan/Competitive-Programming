...
ll numberOfWays(int i, int sum, vector<vector<ll>> &dp){
	...
	ll take = numberOfWays(i, sum - pal[i], dp);
	ll notTake = numberOfWays(i - 1, sum, dp);
...
int main(){
    ...
    vector<vector<ll>> dp(N, vector<ll>((int)pal.size(), -1LL));
    ...
    while(t--){
    	...
    	// auto it = upper_bound(all(pal), n);
    	// ll ind = it - pal.begin();

    	
    	
    	cout << numberOfWays((int)pal.size() - 1, n, dp) % MOD << endl;

    	// for(auto it : pal) cout << it << endl;
    }
    return 0;
}