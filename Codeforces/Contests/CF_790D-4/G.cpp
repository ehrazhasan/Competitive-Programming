...
pair<int,int> dfs(int v, int p){
	...
	for(auto u : edges[v]){
		...
		if(!visited[u] and u != p){
			...
			pair<int,int> ab = dfs(u, v);
			sub = {sub.first + ab.first, sub.second + ab.second};
		}
	}

	char color = s[v - 1];

	pair<int, int> ans1 = sub;
	if(color == 'W') ans1 = {sub.first + 1, sub.second};
	else ans1 = {sub.first, sub.second + 1};

	//db3(v, ans1.first, ans1.second);

	if(ans1.first == ans1.second and ans1.first != 0) ans++;
	return ans1;
}
